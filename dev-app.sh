...
if [ $? != 0 ]; then
    ...
    tmux send-keys -t $SESH:editor "cd ~/projects/direct/" C-m
    tmux send-keys -t $SESH:editor "nvim ." C-m

    tmux new-window -t $SESH -n "server"
    tmux send-keys -t $SESH:server "cd ~/projects/direct/" C-m
    ...
    tmux send-keys -t $SESH:logs "tail -f ~/projects/direct/"
